...
sub run {
  ...
  my $allele = $tva->variation_feature_seq;
    my $threshold = $something->{af_threshold};
    my $highest_threshold;


  my $existing = $vf->{existing};
  my @keys = @{$self->{user_params}->{af_keys}};
  foreach my $existing_var (@$existing) {
    my @frequencies = map {! defined $_ ? "X:0" : $_} @{$existing_var}{@keys};
    my $highest_frequency = get_highest_frequency_vep_cache(\@frequencies, $allele, $lowest_threshold); 
    # last if we found a real frequency which is higher than the lowest threshold    
  }

  foreach my $vcf_collection (@{$self->{config}->{vcf_collections}}) {
    my $alleles = $vcf_collection->get_all_Alleles_by_VariationFeature($vf);
    # sort by frequencies, highest first 
    foreach my $allele (@$alleles) {
        my $af_key = $allele->population->name;
        my $freq = $allele->frequency;
        # exceeds_threshold();
        print STDERR "$af_key $freq\n";
    }
