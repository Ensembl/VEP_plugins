...
sub run {
  ...
  foreach my $data_value (@data) {
    ...
    if (@$matches) {
      ...
      if($output_vcf || $self->{config}->{output_format} eq "json" || $self->{config}->{output_format} eq "rest")  {
      ...
      return ($self->{config}->{output_format} eq "json" || $self->{config}->{output_format} eq "rest") ?  {SpliceAI => \%hash} : \%hash;
    }
  }
